...
export default function Modal({ todo, modalActive, setModalActive }) {
  ...
  const handleNoteSubmit = (e, letter) => {
    if (e.key === "Enter") {
      ...
      if (letter) {
        setNotes([
          ...notes,
          { id: Date.now(), title: letter, completed: false },
        ]);
        setLetter("");
      }
  ...
  return (
    <div
      className={modalActive ? `${cl.modal} ${cl.modal_active}` : `${cl.modal}`}
      onClick={() => handleClickOut()}
    >
      <div
        className={
          modalActive
            ? `${cl.modal__row} ${cl.modal__row_active}`
            : cl.modal__row
        }
      ...
      >
        ...
        <div
          className={cl.tool__container}
          style={
            background.background === "#252525"
              ? {
                  borderLeft: "1px solid #757373",
                  borderRight: "1px solid #757373",
                  borderBottom: "1px solid #757373",
                }
              : { border: "1px solid transparent" }
          }
        >
